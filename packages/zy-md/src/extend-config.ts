...
export function loadExtendConfig(): Partial<ExtendConfig> {
  const paths = [
    path.join(process.cwd(), ".zy-skills-plugin", "zy-markdown-to-html", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(homedir(), ".config"),
      "zy-skills-plugin", "zy-markdown-to-html", "EXTEND.md"
    ),
    path.join(homedir(), ".zy-skills-plugin", "zy-markdown-to-html", "EXTEND.md"),
  ];
